...
/**
 * OSGi component responsible for registering the EXTERNAL governance validation engine.
 */
@Component(
...
public class ExternalGovernanceComponent {
    ...
    protected void activate(ComponentContext componentContext) {
        if (activated) {
            if (log.isDebugEnabled()) {
                log.debug("External governance component already activated, skipping");
            }
            return;
        }

        externalGovernanceValidationEngine = new ExternalGovernanceValidationEngine();
        ValidationEngineFactory.registerValidationEngine(RuleCategory.EXTERNAL, externalGovernanceValidationEngine);
        activated = true;
        log.info("Registered ExternalGovernanceValidationEngine for EXTERNAL rule category");
    }
    ...
    protected void deactivate(ComponentContext componentContext) {
        ...
        activated = false;
        log.info("Unregistered ExternalGovernanceValidationEngine for EXTERNAL rule category");
    }
}
